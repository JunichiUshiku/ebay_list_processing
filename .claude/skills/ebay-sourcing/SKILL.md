---
name: ebay-sourcing
description: eBay販売商品の国内仕入れ先検索ワークフロー。国内11サイトで同一商品を検索し、希望価格・コンディション条件に合う仕入れ先を見つけてスプレッドシートに記録する。
---

# eBay在庫仕入れ処理

## 🚨 必須ルール

- **11サイト全て**を検索対象とする
- eBay商品と**同一品**であることを画像・タイトルで確認
- **価格条件・コンディション条件**を厳守
- **データ取得制約（重要）**: トークン制限回避のため、必要最小限の列のみを取得
  - ❌ 禁止例: `range: C:Y`、`range: A:Z` のような広範囲指定
  - ✅ 正解例: `range: C:C`、`range: E15:E15` のような特定列・特定行のみ

---

## 検索サイト一覧（11サイト）

`##SEARCH##` をキーワードに置換して使用。

| サイト | 検索URL |
|--------|---------|
| Amazon | `https://www.amazon.co.jp/s?k=##SEARCH##` |
| メルカリ | `https://www.mercari.com/jp/search/?keyword=##SEARCH##` |
| フリル | `https://fril.jp/search/##SEARCH##` |
| PayPayフリマ | `https://paypayfleamarket.yahoo.co.jp/search/##SEARCH##?page=1` |
| ヤフオク | `https://auctions.yahoo.co.jp/search/search?va=##SEARCH##` |
| 駿河屋 | `https://www.suruga-ya.jp/search?category=&search_word=##SEARCH##` |
| ハードオフ | `https://netmall.hardoff.co.jp/search/?q=##SEARCH##` |
| ツールオフ | `https://tooloff-ec.com/search/?keyword=##SEARCH##` |
| モノタロウ | `https://www.monotaro.com/s/?c=&q=##SEARCH##` |
| じゃんぱら | `https://www.janpara.co.jp/sale/search/result/?KEYWORDS=##SEARCH##` |
| ジモティー | `https://jmty.jp/all/sale?button=&keyword=##SEARCH##` |

---

## スプレッドシート定義

**Spreadsheet ID**: `1pmbzGCHCqd0EiyuJBl6rfUEGXVITcBDMGPg9bQ67d-g`
**Sheet名**: `AI作業用`

| 列 | 内容 | 備考 |
|----|------|------|
| A | 店舗名/ID | 検索対象の識別子 |
| B | 商品名 | 検索キーワードとして使用 |
| C | eBay Item Number | eBayページ確認用 |
| D | 仕入URL | 既存の仕入れ先URL |
| F | 仕入価格（円） | 空/0→スキップ |
| P | メモ（コンディション） | 新品/中古 |
| U | 結果URL | 値あり→スキップ（処理済み） |
| V | その他候補URL | 最大5件、スラッシュ区切り |
| W | 備考 | 価格、状態、エラー等 |

ヘッダー: 1行目、データ: 2行目から

---

## ワークフロー

### Step 1: TodoWriteでタスク登録（必須・最初に実行）

**⚠️ このステップを最初に実行すること。スキップ禁止。**

TodoWriteツールで以下のタスクを登録:

```
TodoWrite:
- スプレッドシートからデータ取得 (pending)
- eBayページで商品情報確認 (pending)
- 国内11サイトで仕入れ先検索 (pending)
- 結果をスプレッドシートに記録 (pending)
- サマリー報告を出力 (pending)
```

**重要**: 各ステップ完了時に `status: completed` に更新すること。

### Step 2: スキル状態記録

**TodoWrite更新**: `スキル状態を記録` → `in_progress`

```bash
cat > ~/.claude/skill-state.json << 'EOF'
{
  "skill": "ebay-sourcing",
  "message": "スキル ebay-sourcing を再実行し、未処理の行から仕入れ先検索を再開してください。"
}
EOF
```

**TodoWrite更新**: `スキル状態を記録` → `completed`

### Step 3: 商品情報取得（MCP経由）

**TodoWrite更新**: `スプレッドシートからデータ取得` → `in_progress`

```
mcp__google-sheets__get_sheet_data
spreadsheet_id: 1pmbzGCHCqd0EiyuJBl6rfUEGXVITcBDMGPg9bQ67d-g
sheet: AI作業用
range: A2:W
```

**スキップ判定**（U列の値を確認）:
- URL形式（`https://`）or「なし」or「スキップ」→ 処理済み、次の行へ
- 数値のみ（「0」「1」等）or 空 → **未処理、検索実行**

**F列が空/0** → U列に「スキップ」記載して次の行へ

**TodoWrite更新**:
- `スプレッドシートからデータ取得` → `completed`
- `処理対象行の特定` → `in_progress` → `completed`

### Step 4: eBayページ確認

**TodoWrite更新**: `eBayページで商品情報確認` → `in_progress`

- URL: `https://www.ebay.com/itm/{C列}`
- 商品画像・タイトルを確認（同一商品判断用）
- ページ存在しない → W列に記載してスキップ
- ENDED（売り切れ）でも仕入れ先は探す

### Step 5: 検索キーワード確定

- B列に値あり → そのまま使用
- B列が空 → eBayタイトル・画像から日本語キーワードを推測

### Step 6: 国内11サイト一括検索

**TodoWrite更新**: `国内11サイトで仕入れ先検索` → `in_progress`

上記「検索サイト一覧」の11サイトURLを使用。

1. **URL生成**: `##SEARCH##`をキーワードに置換
2. **一括オープン**: 全URLを新しいタブで開く
3. **順次確認**: 各タブを確認

#### トークン削減ルール（効率的な検索処理）

**0件検出の高速化**:
- 検索結果ページで「0件」「該当なし」「見つかりません」を即判定
- 該当時は詳細確認せず即スキップ

**価格フィルタリング**:
- 検索結果一覧で価格を先に確認
- 希望価格（F列）×1.5倍以上の商品は詳細確認スキップ

**コンディション事前判定**:
- 検索結果一覧で「ジャンク」「難あり」「破損」を含む商品は除外
- 詳細ページ訪問は条件合致候補のみ

**javascript_toolによる情報抽出**:
- `read_page`の代わりに`javascript_tool`で必要情報のみ抽出
- 抽出対象: 検索結果件数、商品タイトル（上位10件）、価格一覧

**確認ルール**:
- ヒット数0件 → 即スキップ（タブを閉じる）
- ヒットあり → 価格・コンディションフィルタ適用後、条件合致候補のみ詳細確認
- 見つからない場合はキーワード調整して再検索可

### Step 7: 仕入れ判断

**同一商品判断**:
- eBay画像・タイトルと照合
- 色違い・サイズ違い → 別商品
- セット商品のバラ売り → 対象外

**価格条件**:
- F列の希望価格以下（税込み・送料込み）
- 送料不明時 → +1300円で計算
- ポイント還元は考慮しない

**オークション判断**:
- 即決商品優先
- 現在価格が希望価格内ならOK（即決あればそちら優先）
- 入札中商品も対象

**コンディション条件**:
| P列 | 判断 |
|-----|------|
| 「中古」or 空 | 新品・中古どちらもOK |
| 「新品」 | 新品優先、なければ中古可（W列に記載） |

| 表記 | 扱い |
|------|------|
| 「未使用に近い」 | 新品扱い |
| 「目立った傷や汚れなし」 | 中古扱い |
| 「難あり」「訳あり」「破損」「欠品」「ジャンク」 | 除外 |
| 「箱なし」「説明書なし」 | 他に候補なければ適用可 |
| 売り切れ商品 | 除外 |

### Step 8: 結果記録（MCP経由）

**TodoWrite更新**: `結果をスプレッドシートに記録` → `in_progress`

```
mcp__google-sheets__update_cells
spreadsheet_id: 1pmbzGCHCqd0EiyuJBl6rfUEGXVITcBDMGPg9bQ67d-g
sheet: AI作業用
range: U{行番号}:W{行番号}
data: [[U列の値, V列の値, W列の値]]
```

| セル | 記録内容 | 例 |
|------|----------|-----|
| **U{行}** | ベストな商品URL or 「なし」 | `https://auctions.yahoo.co.jp/...` |
| **V{行}** | その他候補URL（最大5件、スラッシュ区切り） | `https://... / https://...` |
| **W{行}** | 備考（価格、状態、エラー等） | `¥26,800(税送込)、中古` |

**重要**: U列に既存の数値（「0」「1」等）があっても**上書きする**

### Step 9: 後処理

1. 検索サイトの不要タブを閉じる
2. 次の商品行へ移動してStep 4〜8を繰り返す
3. **10件ごとに進捗報告**

**TodoWrite更新**:
- `国内11サイトで仕入れ先検索` → `completed`
- `結果をスプレッドシートに記録` → `completed`

### Step 10: 完了処理

**TodoWrite更新**: `サマリー報告を出力` → `in_progress`

**サマリー報告（テーブル形式）**:

| 項目 | 結果 |
|------|------|
| 処理件数 | {処理した件数} |
| 仕入れ先発見 | {発見件数} |
| 見つからず | {見つからなかった件数} |
| スキップ | {スキップ件数} |
| エラー | {エラー件数}（行番号: {該当行}） |

**TodoWrite更新**: `サマリー報告を出力` → `completed`

---

## 実行パラメータ

| 指定 | 動作 |
|------|------|
| 「全て」 | 上から順に全件処理（U列に値ある行はスキップ） |
| アイテムナンバー指定 | 指定行のみ処理 |
| 再処理 | U列を消して再実行 |

---

## 前提条件

- Google Sheets MCPサーバー設定済み
- サービスアカウントがスプレッドシートに共有されている（編集権限）
- Cookie同意ダイアログは「許可」で対応（国内サイト検索時）

---

## javascript_tool情報抽出ガイド

`read_page`の代わりに`javascript_tool`を使用してトークン消費を削減。

### 検索結果件数の取得

```javascript
// 汎用: ページ内の件数表示を抽出
document.body.innerText.match(/(\d+)件/)?.[1] || '0'
```

### サイト別抽出例

**メルカリ**:
```javascript
JSON.stringify({
  count: document.querySelectorAll('[data-testid="item-cell"]').length,
  items: [...document.querySelectorAll('[data-testid="item-cell"]')].slice(0,10).map(el => ({
    title: el.querySelector('[data-testid="thumbnail-item-name"]')?.textContent?.trim(),
    price: el.querySelector('[data-testid="thumbnail-item-price"]')?.textContent?.trim()
  }))
})
```

**ヤフオク**:
```javascript
JSON.stringify({
  count: document.querySelector('.Result__count')?.textContent?.match(/\d+/)?.[0] || '0',
  items: [...document.querySelectorAll('.Product')].slice(0,10).map(el => ({
    title: el.querySelector('.Product__title')?.textContent?.trim(),
    price: el.querySelector('.Product__priceValue')?.textContent?.trim()
  }))
})
```

**Amazon**:
```javascript
JSON.stringify({
  count: document.querySelector('[data-component-type="s-result-info-bar"]')?.textContent?.match(/\d+/g)?.[0] || '0',
  items: [...document.querySelectorAll('[data-component-type="s-search-result"]')].slice(0,10).map(el => ({
    title: el.querySelector('h2')?.textContent?.trim(),
    price: el.querySelector('.a-price-whole')?.textContent?.trim()
  }))
})
```

### 0件判定の高速化

```javascript
// 0件の場合true、それ以外false
document.body.innerText.includes('0件') ||
document.body.innerText.includes('見つかりません') ||
document.body.innerText.includes('該当する商品がありません')
```
