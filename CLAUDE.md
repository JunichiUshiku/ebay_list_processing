...
### サブエージェント共通ブラウザ管理ルール（必須）

全マッチャーエージェント（mercari / paypay / rakuma / yahoo / hardoff / surugaya）に適用：

1. **処理完了時に必ずセッションを閉じる**: `agent-browser --session {自分のセッション名} close`
2. **エラー終了時も必ず閉じる**: JSON返却の直前に必ず close を実行
3. **セッション名は固定**: 番号付き（`mercari2`, `rakuma3` 等）や別名セッションの作成禁止
4. **他エージェントのセッション名を使用禁止**: 自分以外のセッション（`yahoo`, `paypay` 等）を使ってはならない
5. **`--headed` オプション禁止**: サブエージェントはヘッドレスモードで動作すること
6. **`pkill` によるChrome/agent-browser強制終了禁止**: 他エージェントのセッションを巻き込むため

| エージェント | セッション名 |
|------------|------------|
| mercari-matcher | `mercari` |
| paypay-matcher | `paypay` |
| rakuma-matcher | `rakuma` |
| yahoo-auction-matcher | `yahoo` |
| hardoff-matcher | `hardoff` |
| surugaya-matcher | `surugaya` |
